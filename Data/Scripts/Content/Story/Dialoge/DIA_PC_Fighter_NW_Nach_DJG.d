...
instance DIA_GornNW_nach_DJG_AllDragonsDead(C_INFO)
{
	...
	permanent		= TRUE;
	description		= "Entspannst du dich?";
};
...
{
	AI_Output(other, self, "DIA_GornNW_nach_DJG_AllDragonsDead_15_00"); //Entspannst du dich?
	AI_Output(self, other, "DIA_GornNW_nach_DJG_AllDragonsDead_12_01"); //Klar, warum nicht? Die Drachen sind doch tot, oder?
	AI_Output(other, self, "DIA_GornNW_nach_DJG_AllDragonsDead_15_02"); //Der Fall ist ein wenig komplizierter.
	AI_Output(self, other, "DIA_GornNW_nach_DJG_AllDragonsDead_12_03"); //(lacht) Tja. Wie immer. Sag Bescheid, wenn du meine Axt brauchst.
};
...
instance DIA_GornNW_nach_DJG_KnowWhereEnemy(C_INFO)
{
	...
	permanent		= TRUE;
	description		= "Ich könnte dich und deine Axt gebrauchen.";
};
...
{
	AI_Output(other, self, "DIA_GornNW_nach_DJG_KnowWhereEnemy_15_00"); //Ich könnte dich und deine Axt gebrauchen.
	AI_Output(self, other, "DIA_GornNW_nach_DJG_KnowWhereEnemy_12_01"); //Na. Das wundert mich nicht. Was kann ich für dich tun?
	AI_Output(other, self, "DIA_GornNW_nach_DJG_KnowWhereEnemy_15_02"); //Würdest du mit mir zu einer Insel segeln und dem Feind in den Arsch treten?
	...
	if (crewmember_count >= Max_Crew)
	{
		...
		AI_Output(self, other, "DIA_GornNW_nach_DJG_KnowWhereEnemy_12_05"); //Wenn jemand anders wegen mir wieder aus deiner Mannschaft fliegt, dann nimm den Schwächsten von ihnen.
		AI_Output(self, other, "DIA_GornNW_nach_DJG_KnowWhereEnemy_12_06"); //In diesen rauen Zeiten brauchst jeden Mann, der anpacken kann.
	}
	else
	{
		Info_ClearChoices(DIA_GornNW_nach_DJG_KnowWhereEnemy);
		Info_AddChoice(DIA_GornNW_nach_DJG_KnowWhereEnemy, "Ich sag bescheid, wenn´s los geht.", DIA_GornNW_nach_DJG_KnowWhereEnemy_No);
		Info_AddChoice(DIA_GornNW_nach_DJG_KnowWhereEnemy, "Komm zu mir an Bord.", DIA_GornNW_nach_DJG_KnowWhereEnemy_Yes);
	};
};

func void DIA_GornNW_nach_DJG_KnowWhereEnemy_Yes()
{
	AI_Output(other, self, "DIA_GornNW_nach_DJG_KnowWhereEnemy_Yes_15_00"); //Komm zu mir an Bord. Warte am Hafen auf mich.
	AI_Output(self, other, "DIA_GornNW_nach_DJG_KnowWhereEnemy_Yes_12_01"); //Ein Schiff? Ha! Eins muss man dir lassen. Mit dir wird es nie langweilig. Komm schnell nach.
...
{
	AI_Output(other, self, "DIA_GornNW_nach_DJG_KnowWhereEnemy_No_15_00"); //Ich sag Bescheid, wenn's los geht.
	AI_Output(self, other, "DIA_GornNW_nach_DJG_KnowWhereEnemy_No_12_01"); //In Ordnung.
...
instance DIA_GornNW_nach_DJG_LeaveMyShip(C_INFO)
{
	...
	permanent		= TRUE;
	description		= "Vielleicht solltest du besser hier bleiben.";
};
...
{
	AI_Output(other, self, "DIA_GornNW_nach_DJG_LeaveMyShip_15_00"); //Vielleicht solltest du besser hier bleiben.
	AI_Output(self, other, "DIA_GornNW_nach_DJG_LeaveMyShip_12_01"); //Ich soll dich alleine gehen lassen? Mmh. Wird mir schwer fallen, aber es ist dein Krieg. Komm zu mir, wenn ich dir doch folgen soll.
...
instance DIA_GornNW_nach_DJG_StillNeedYou(C_INFO)
{
	...
	permanent		= TRUE;
	description		= "Komm zurück. Ich brauche dich.";
};
...
{
	AI_Output(other, self, "DIA_GornNW_nach_DJG_StillNeedYou_15_00"); //Komm zurück. Ich brauche dich.
	AI_Output(self, other, "DIA_GornNW_nach_DJG_StillNeedYou_12_01"); //Na also. Und ich dachte schon, du willst mich hier vergammeln lassen, während du den ganzen Spaß hast. Bis später.
