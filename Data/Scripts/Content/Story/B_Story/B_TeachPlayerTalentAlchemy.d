...
func int B_TeachPlayerTalentAlchemy(var C_Npc slf, var C_Npc oth, var int potion)
{
	...
	B_LogEntry(TOPIC_TalentAlchemy,"Ingredients 'ESSENCE OF HEALING':2 Healing Plants, 1 Forest Berry."); };

	if (potion == POTION_Health_02) { PLAYER_TALENT_ALCHEMY[POTION_Health_02] = TRUE;
	B_LogEntry(TOPIC_TalentAlchemy,"Ingredients 'EXTRACT OF HEALING':2 Healing Herbs, 1 Blue Elder.");  };

	if (potion == POTION_Health_03) { PLAYER_TALENT_ALCHEMY[POTION_Health_03] = TRUE;
	B_LogEntry(TOPIC_TalentAlchemy,"Ingredients 'ELIXIR OF HEALINGÃƒâ€šÃ‚Â´:2 Healing Roots, 1 Meadow Knotweed.");  };

	if (potion == POTION_Mana_01) { PLAYER_TALENT_ALCHEMY[POTION_Mana_01] = TRUE;
	B_LogEntry(TOPIC_TalentAlchemy,"Ingredients 'MANA ESSENCE': 2 Fire Nettles, 1 Meadow Berry.");  };

	if (potion == POTION_Mana_02) { PLAYER_TALENT_ALCHEMY[POTION_Mana_02] = TRUE;
	B_LogEntry(TOPIC_TalentAlchemy,"Ingredients 'MANA EXTRACT': 2 Fireweeds, 1 Blue Elder.");  };

	if (potion == POTION_Mana_03) { PLAYER_TALENT_ALCHEMY[POTION_Mana_03] = TRUE;
	B_LogEntry(TOPIC_TalentAlchemy,"Ingredients 'MANA ELIXIR':2 Fire Roots, 1 Meadow Knotweed.");  };
	...
	B_LogEntry(TOPIC_TalentAlchemy,"Ingredients 'ELIXIR OF STRENGTH':1 Dragonroot, 1 King's Sorrel and 1 Meadow Knotweed.");  };

	if (potion == POTION_Perm_DEX) { PLAYER_TALENT_ALCHEMY[POTION_Perm_DEX] = TRUE;
	B_LogEntry(TOPIC_TalentAlchemy,"Ingredients 'ELIXIR OF DEXTERITY': 1 Goblin Berries, 1 King's Sorrel and 1 Meadow Knotweed.");  };

	if (potion == POTION_Perm_Mana) { PLAYER_TALENT_ALCHEMY[POTION_Perm_Mana] = TRUE;
	B_LogEntry(TOPIC_TalentAlchemy,"Ingredients 'ELIXIR OF SPIRIT':1 Fire Root, 1 King's Sorrel and 1 Meadow Knotweed.");  };

	if (potion == POTION_Perm_Health) { PLAYER_TALENT_ALCHEMY[POTION_Perm_Health] = TRUE;
	B_LogEntry(TOPIC_TalentAlchemy,"Ingredients 'ELIXIR OF LIFE':1 Healing Root, 1 King's Sorrel and 1 Meadow Knotweed.");  };
