...
/* INSTANCE PC_PrayShrine_BlessRegen (C_Info)
{
	npc				= PC_Hero;
	nr				= 3;
	condition		= PC_PrayShrine_BlessRegen_Condition;
	information		= PC_PrayShrine_BlessRegen_Info;
	permanent		= TRUE;
	description		= GMM_Bless_Regen; 
};

FUNC INT PC_PrayShrine_BlessRegen_Condition ()
{
	if (PLAYER_MOBSI_PRODUCTION	==	MOBSI_PRAYSHRINE)
	&& (hero.guild == GIL_KDF)
	&& (Npc_GetDistToWP (hero,"NW_MONASTERY_CHAPELL_02") <= 500)   
	&& ((Npc_HasItems (hero,ItMW_GMM_Stab_02_Regen) >= 1))
	{	
		return TRUE;
	};
};

FUNC VOID PC_PrayShrine_BlessRegen_Info()
{
	if (ShrineIsObsessed == TRUE)
	{
		SC_IsObsessed = TRUE;
	...
	{	
		if (Npc_HasItems (hero,ItMi_Gold) >= Gold_BlessRegen_GMM)
		{
			Npc_RemoveInvItems  (hero,ItMi_Gold, Gold_BlessRegen_GMM);
			concatDonation = ConcatStrings(IntToString(Gold_BlessRegen_GMM), PRINT_GoldGegeben);				
			AI_PrintScreen	(concatDonation, -1, YPOS_GoldGiven, FONT_ScreenSmall, 2);
			
			if (Npc_HasItems (hero,ItMW_GMM_Stab_02_Regen) >= 1)	
			{	
				Npc_RemoveInvItems  (hero,ItMW_GMM_Stab_02_Regen, 1);
				CreateInvItems 		(hero,ItMW_GMM_Stab_02_Regen_Blessed, 1);
				Wld_PlayEffect("spellFX_PalHeal_ORIGIN",  hero, hero, 0, 0, 0, FALSE );
				Snd_Play ("MFX_Heal_Cast" );
				B_GivePlayerXP (XP_SwordBlessed);
			};
