...
// Kapitel 1 ADDON
	...
	const int XP_Addon_BringMissingOrnament				= 200;									// fÃƒÂ¼r jedes der 3 Ornamente
	const int XP_Addon_Martin_GotRangar_Report			= 100;									// (nur MilizanwÃƒÂ¤rter)
	const int XP_Addon_Martin_GotRangar					= 300;									// (nur MilizanwÃƒÂ¤rter)
	const int XP_Addon_ReturnedLostInnosStatue_Daron	= 300;									// (nur KDFanwÃƒÂ¤rter)
	const int XP_Addon_ReportLostInnosStatue2Daron		= 100;									// (nur KDFanwÃƒÂ¤rter)
	const int XP_Addon_Cord_Look4Patrick				= 300;									// (nur SLDanwÃƒÂ¤rter)
	const int XP_Addon_Dexter_KnowsPatrick				= 100;									// (nur SLDanwÃƒÂ¤rter)
	...
	const int XP_Addon_BringStoneplate					= 50;									// fÃƒÂ¼r jede Steintafel
	...
	const int XP_Addon_Vatras_MissingPeopleReport		= 50;									// pro Report bei Vatras fÃƒÂ¼r die missing People
	const int XP_Addon_AllNWTeleporststones				= 200;
	const int XP_Greg_NW_GiveBauArmor					= 100;

// Kapitel 2 ADDON
	const int XP_Addon_LookForMorgan					= 100;									// FÃƒÂ¼r AlligatorJack Moran suchen
	...
	const int XP_Addon_HenryProofCrew					= 200;									// ÃƒÅ“berzeuge HenrysMÃƒÂ¤nner
	const int XP_Addon_MorganProofCrew					= 200;									// ÃƒÅ“berzeuge MorgansMÃƒÂ¤nner
	...
	const int XP_Addon_Riordian_FoundHouse				= 100;									// 5 mal fÃƒÂ¼r jedes Haus einen
	const int XP_addon_ClearCanyon						= 250;
	const int XP_Bones_GetBDTArmor						= 200;

// Kapitel 3 ADDON
	const int XP_Addon_Andre_MissingPeople				= 600;									// (nur MIL oder PAL)Report bei Andre fÃƒÂ¼r Returning Missing People.
	const int XP_Addon_ForOneToken						= 400;									// fÃƒÂ¼r jede der 5 Steintafeln (Token) an Saturas abgegeben.
	const int XP_Addon_Myxir_GeistBeschwoeren			= 400;									// SC berichtet von geglÃƒÂ¼ckter GeistbeschwÃƒÂ¶rung.
	...
	const int XP_Addon_ActivatedTeleportStone			= 500;									// (5 mal) fÃƒÂ¼r jeden aktivierten Teleportstein. bei Merdarion.
	...
	const int XP_Addon_SkipsGrog						= 200;									// Skip den Grog zurÃƒÂ¼ck gegeben.
	const int XP_Addon_Morgan_LurkerPlatt				= 300;									// SC sÃƒÂ¤ubert Strand fÃƒÂ¼r Morgan.
...
// -----------------------------------------------------------------
// Banditenlager Addon Kapitel 2? Drei? Wer weiÃƒÅ¸ es?
// -----------------------------------------------------------------
	...
	const int XP_Addon_MINE								= 150;									// 3x fÃƒÂ¼r Buddler in die mine schicken
	...
	const int XP_Addon_HunoStahl						= 200;									// Stahl fÃƒÂ¼r Huno
	const int XP_Addon_LennarPaket						= 200;									// Dietriche fÃƒÂ¼r lennar
	const int XP_Addon_Tom								= 100;									// Tom nicht verpetzen

	const int XP_Addon_Auftraggeber						= 500;									// Auftraggeber gefunden (Snaf)
	const int XP_Addon_Loushammer						= 150;									// Schnapsbrennen fÃƒÂ¼r Snaf
...
// ------ AMBIENT XP -----
	var int XP_Ambient;																			// kann fÃƒÂ¼r alle kleinen Ambient Sachen verwendet werden M.F.
...
// ------ Kapitel 1 ------
	...
	const int XP_BringOldCoin							= 10;									// Joly: fÃƒÂ¼r jede alte MÃƒÂ¼nze, die SC zu Wasili bringt. (kann bis zu 800 XP bringen! bis Kapitel 5 )
	...
	const int XP_Andre_Auslieferung						= 100;									// gibt's fÃƒÂ¼r jeden ausgelieferten NPC
...
// ------ Kapitel 3 ------
	...
	const int XP_HannaRetrieveLetter					= 300;									// fÃƒÂ¼r alle
	const int XP_HelpDiegoNW							= 600;									// Finde Diegos Schatz
	const int XP_DiegosResidence						= 400;									// ein neues Haus fÃƒÂ¼r Diego
	...
	const int XP_BringSerpentesMinenAnteil				= 75;									// FÃƒÂ¼r jedes Minenanteil, dass SC zu Serpentes bringt, 10 X XP_BringSerpentesMinenAnteil = 750 (nur PAL)
	const int XP_SalandrilInKloster						= 400;									// Salandril bÃƒÂ¼hst seine SÃƒÂ¼nden im Kloster (KDF und SLD,DJG)
	const int XP_Akil_SchafDiebe						= 400;									// SC hat Schafdiebe getÃƒÂ¶tet
	const int XP_AkilsSchaf								= 200;									// Schaf ist wieder da.
	const int XP_FoundVinosKellerei						= 400;									// Moleratefett anngewendet und VinosKellerei geÃƒÂ¶ffnet
	...
	const int XP_Lester_KnowsMadPsi						= 300;									// Lester weiÃƒÅ¸ jetzt, dass die Suchenden vorher Psioniker waren (KDF)
	const int XP_Angar_KnowsMadPsi						= 300;									// Angar weiÃƒÅ¸ jetzt, dass die Suchenden vorher Psioniker waren (KDF)
	...
	const int XP_KillMorgahard							= 300;									// SC tÃƒÂ¶tet Morgahard fÃƒÂ¼r den Richter. (SLD)
	const int XP_FoundMorgahard							= 500;									// Morgahard gefunden (SLD)
	const int XP_SwordBlessed							= 400;									// Schwert wurde geweiht
	const int XP_Torlof_DMT								= 300;									// fÃƒÂ¼r torlof den DMT getÃƒÂ¶tet
...
// ------ Kapitel 4 ------
	...
	const int XP_KDF_BringAlmanach						= 450;									// Joly: nur Kdf XP fÃƒÂ¼r einen gefundenen Almanach -> ca. 10 x XP_KDF_BringAlmanach
...
// ------ Kapitel 5 ------
	...
	const int XP_Crewmember_Success						= 500;									// FÃƒÂ¼r jedes Crewmitglied!!
	const int XP_Captain_Success						= 2000;									// ich habe einen KapitÃƒÂ¤n lalalalala
