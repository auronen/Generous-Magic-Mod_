// **************************
// AIVAR-Kennungen fÃƒÂ¼r Humans
// **************************

// ------ NEWS und MEMORY Aivars ------
// - NUR Augenzeugen
// - NUR persÃƒÂ¶nliches News-GedÃƒÂ¤chtnis, geregelt ÃƒÂ¼ber die folgenden Aivars
// - schwere Tat ÃƒÂ¼berschreibt leichte Tat
// - KEINE automatische News-Verbreitung, AIV-Check bei aNSCs in Dialogen = Ersatz fÃƒÂ¼r News-Verbreitung
// - News-Vergabe ÃƒÂ¼ber Dialog-Module
	...
	const int CRIME_ATTACK								= 2;									// Kampf self-SC --> feinere Analys in DIA-Modul ÃƒÂ¼ber AIV_LastFightAgainstPlayer
...
// ------ ZS_Talk ------------------------------
	const int AIV_INVINCIBLE							= 4;									// ist TRUE fÃƒÂ¼r beide Teilnehmer eines Dialogs
...
// ------ B_SayAttackReason --------------------
	...
	const int AR_MonsterVsHuman							= 4;									// Monster kÃƒÂ¤mpft gegen Human - ich helfe Human
	const int AR_MonsterMurderedHuman					= 5;									// Monster hat Human getÃƒÂ¶tet
	const int AR_SheepKiller							= 6;									// Schaf wurde angegriffen oder getÃƒÂ¶tet (von Mensch oder Monster)
	...
	const int AR_ReactToWeapon							= 10;									// TÃƒÂ¤ter hat trotz zweimaliger Warnung Waffe nicht weggesteckt ODER ich fliehe direkt
	const int AR_ReactToDamage							= 11;									// TÃƒÂ¤ter hat mich verletzt
	const int AR_GuardStopsFight						= 12;									// Wache beendet Kampf, greift TÃƒÂ¤ter an
	const int AR_GuardCalledToKill						= 13;									// Wache durch WARN zum Mit-TÃƒÂ¶ten gerufen
	...
	const int AR_SuddenEnemyInferno						= 18;									// EnemyOverride Blockierung fÃƒÂ¼r mich selbst und alle NPCs im Umkreis aufheben.
	const int AR_KILL									= 19;									// Spieler aus Dialog heraus tÃƒÂ¶ten (SC hat keine Chance)

// ------ ZS_RansackBody ------------------------
	const int AIV_RANSACKED								= 10;									// damit nur EIN NSC einen Body plÃƒÂ¼ndert

// ------ ZS_Dead -------------------------------
	const int AIV_DeathInvGiven							= 11;									// FÃƒÂ¼r Mensch und Monster!

// ------ TA_GuardPassage -----------------------
	const int AIV_Guardpassage_Status					= 12;									// EnthÃƒÂ¤lt den Status einer Durchgangswache
	const int GP_NONE									= 0;									// ...Anfang; SC noch nicht in der NÃƒÂ¤he
	...
	const int AIV_LastDistToWP							= 13;									// enthÃƒÂ¤lt die letzte ermittelte Distanz zum Referenz-Waypoint (Checkpoint) (wird beim SPIELER gesetzt!)
	const int AIV_PASSGATE								= 14;									// TRUE = Spieler kann durch, FALSE = Spieler wird aufgehalten

// ------ XP-Vergabe ----------------------------
	const int AIV_PARTYMEMBER							= 15;									// fÃƒÂ¼r XP-Vergabe, AUCH fÃƒÂ¼r Monster
	const int AIV_VictoryXPGiven						= 16;									// wenn NSC schonmal besiegt, gibts keine XP mehr fÃƒÂ¼r den Spieler (z.B. bei unconscious Humans)

// ------ Geschlecht ----------------------------
	const int AIV_Gender								= 17;									// wird zusammen mit Visual ÃƒÂ¼ber B_SetVisual gesetzt
	const int MALE										= 0;
	const int FEMALE									= 1;
...
// ------ TA Hilfsvariable ----------------------- // notwendig da lokale Hilfsvariablen anscheinend nicht immer neu initialisiert werden bitte nur fÃƒÂ¼r TAs verwenden!
...
// ***************************
// AIVAR-Kennungen fÃƒÂ¼r Monster
// ***************************
	...
	const int AIV_MM_FollowInWater						= 28;									// AUCH fÃƒÂ¼r HUMANS! - folgt einem der NSC auch in Wasser?
// ----------------------------------------------
	const int AIV_MM_PRIORITY							= 29;									// ist meine PrioritÃƒÂ¤t gerade FRESSEN oder KÃƒâ€žMPFEN (immer fressen, es sei denn ich werde getroffen)
...
// ------ zusÃƒÂ¤tzlich zur CRIME merken -----------
...
// ------ ToughGuy IGNORIERT Attack-Crime ------
	const int AIV_ToughGuy								= 57;									// Jubelt beim Kampf, hat keine News bei Attack (kann ÃƒÂ¼ber AIV_LastFightAgainstPlayer reagieren)
...
// ------ diverse ZS -------
	const int AIV_StateTime								= 68;									// um mit zwei StateTime-Triggern arbeiten zu kÃƒÂ¶nnen.
...
// -------------------------
	...
	const int AIV_NPCIsRanger							= 72;									// Der Typ gehÃƒÂ¶rt zum 'Ring des Wassers'
	const int AIV_IgnoresArmor							= 73;									// Keine Reaktion oder Konsequenzen auf die RÃƒÂ¼stung des SC
	const int AIV_StoryBandit							= 74;									// Banditen, mit denen gekÃƒÂ¤mpft werden darf
...
// ---------------------------------------------
	...
	const int PERC_DIST_HEIGHT							= 1000;									// ab welchem HÃƒÂ¶henunterschied wird Wn ignoriert
	const int PERC_DIST_INDOOR_HEIGHT					= 250;									// dasselbe fÃƒÂ¼r Indoor (zum Ignorieren von anderen Stockwerken wenn ganzes Haus (HÃƒÂ¶hlensystem) EIN Portalraum ist)
// ---------------------------------------------
	const int FIGHT_DIST_MELEE							= 600;									// Bis zu welcher Entfernung Bedrohung durch SC-Waffe
	const int FIGHT_DIST_RANGED_INNER					= 900;									// Ab welcher Entfernung NK wÃƒÂ¤hlen, wenn NSC im FK ist
	const int FIGHT_DIST_RANGED_OUTER					= 1000;									// Ab welcher Entfernung FK wÃƒÂ¤hlen, wenn NSC im NK ist oder Waffe zum ersten Mal gezogen wird
...
** Zeit - Konstanten *
********************************************************************/
	const int NPC_ANGRY_TIME							= 120;									// MUSS SO HEISSEN, ist vom Programm ausgelagert - Spielsekunden, die die Temp_Att aufrechterhalten wird, bevor sie wieder auf Perm_Att gesetzt wird (gilt fÃƒÂ¼r alle At, nicht nur fÃƒÂ¼r angry)
// -------------------------------------------
	const int HAI_TIME_UNCONSCIOUS						= 20;									// MUSS SO HEISSEN, ist vom Programm ausgelagert (Default = 20) - Zeit in Sekunden, die der SC und NSCs bewuÃƒÅ¸tlos bleiben
// -------------------------------------------
...
** Mindestschaden **
********************************************************************/
	const int NPC_MINIMAL_DAMAGE						= 5;									// MUSS SO HEISSEN, ist vom Programm ausgelagert - Untere Genze des Mindestschadens fÃƒÂ¼r Menschen (*** und Monster ??? ***)
	const int NPC_MINIMAL_PERCENT						= 10;									// MUSS SO HEISSEN, ist vom Programm ausgelagert - Mindestschaden wird ermittelt durch X% vom normalen Gesamtschaden (NACH Abzug der RÃƒÂ¼stung), wobei NPC_MINIMAL_DAMAGE genommen wird, falls Mindestschaden NACH %-Berechnung kleiner als NPC_MINIMAL_DAMAGE!
...
// Alles Addon
	...
	const int FAI_Bear									= 45;									// BÃƒÂ¤r
	const int FAI_Stoneguardian							= 46;									// SteinwÃƒÂ¤chter
...
// werden benÃƒÂ¶tigt um eine Unterscheidung der Mobsi beim PC_Hero Dialog vorzunehmen.
...
// *****************************
// Konstanten fÃƒÂ¼r B_SetNpcVisual
// *****************************

// ------ Nacktmesh-Texturen fÃƒÂ¼r MÃƒÂ¤nner und Frauen (je 4) ------
	...
	const int BodyTexBabe_B								= 7;									// Black Babe // Frauen werden auch mit den "MÃƒÂ¤nner"-Konstanten angegeben, dann vom Script +4 addiert, d.h. diese Konstanten werden nicht gebraucht
	const int BodyTex_Player							= 8;

// ---------ADD ON----------------------
	const int BodyTex_T									= 10;									// tÃƒÂ¤towierte psionikerhaut
