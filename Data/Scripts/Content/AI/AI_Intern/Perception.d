...
func void InitPerceptions()
{
	...
	// PERC_ASSESSENEMY // der nÃƒÂ¤chste NSC, zu dem ich Att_Hostile habe, wird wahrgenommen! (wenn Temp_Att != Gil_Att, wird Temp_att genommen)
	...
	// Aktive Wahrnehmungen werden NUR ÃƒÂ¼ber die aktivierten senses getriggert -Humans haben KEIN sense_smell, d.h. ein Aufruf von z.B. B_AssessPlayer beinhaltet schon einen Sichtcheck (etwa 180Ã‚Â° vor dem NSC)

	// *** ACHTUNG *** Passive Wahrnehmungen sind sense-unabhÃƒÂ¤ngig (sie gehen immer an alle NSCs, egal ob's gesehen/gehÃƒÂ¶rt wird oder nicht)
	...
	// wird an alle NSCs gesendet, wenn ein NSC bewuÃƒÅ¸tlos wird
	Perc_SetRange(PERC_ASSESSMURDER, PERC_DIST_ACTIVE_MAX); // eigentlich INTERMEDIAT, ist hier grÃƒÂ¶ÃƒÅ¸er, weil MÃƒÂ¶rder direkt hinter mir, aber Opfer 11m entfernt stehen kann
	...
	Perc_SetRange(PERC_ASSESSFIGHTSOUND, 3000); // wird in fast allen FÃƒÂ¤llen auf PERC_DIST_INTERMEDIAT verkÃƒÂ¼rzt!
	...
	// wird gesendet von VisualFXInst.d - Wahrnehmungs-Sendung hÃƒÂ¤ngt am Magie Effekt
	// Ein NSC, der diese Wahrnehmung bekommt, kann in der AI ZUSÃƒâ€žTZLICH auf Magie reagieren (i.d.R. mit ZS-Wechsel)
	...
	// Wird an das / die Opfer gesendet, wenn der Spell aufhÃƒÂ¶rt zu wirken
	...
	// Wenn NSC eine TÃƒÂ¼r im Weg ist

	// --- SC verwandelt sich / - zurÃƒÂ¼ck ------------------------------------------

	Perc_SetRange(PERC_ASSESSSURPRISE, FIGHT_DIST_CANCEL); // nur in ZS_Attack und ZS_MM_Attack
	// geht an alle, wenn der SC sich zurÃƒÂ¼ckverwandelt

	// --- RAUS (wird nicht gebraucht) -------------------------------------------

	Perc_SetRange(PERC_OBSERVEINTRUDER, 100);
	// geht an alle, wenn SC stehenbleibt (auÃƒÅ¸er aus dem Schleichen heraus)
...
func void Perception_Set_Normal() // FÃƒÂ¼r Humans
{
	// --- Die Senses bei jedem Zustand neu setzen (Weil in ZS_Guide_Player die Senses & Ranges verÃƒÂ¤ndert werden) ------
	...
	Npc_PercEnable(self, PERC_MOVEMOB, B_MoveMob); // ÃƒÂ¶ffnet TÃƒÂ¼ren, die auf dem Weg liegen
};
...
func void Perception_Set_Minimal() // FÃƒÂ¼r MÃƒÂ¤nner und Frauen
{
	// --- Die Senses bei jedem Zustand neu setzen (nur falls in ZS mit Perc_Set_Minimal noch ZUSÃƒâ€žTLZLICH (lokal) aktive Wahrnehmungen angemeldet werden) ------
	self.senses = SENSE_HEAR | SENSE_SEE;
	self.senses_range = PERC_DIST_ACTIVE_MAX;
...
// ÃƒÅ“BERALL DORT aufgerufen werden, wo ein neuer ZS aufgerufen wird
// Beachten: auch in B_Befehlen, die wiederum aus diesem B_Befehlen aufgerufen werden
// mÃƒÂ¼ssen alle ZS_Aufrufe mit B_ClearPerceptions versehen werden
// AUF KEINEN FALL einfach B_ClearPerceptions in jeden B_Befehl, der durch eine Wahrnehmung aufgerufen wird,
// reinmachen (so daÃƒÅ¸ er IMMER aufgerufen wÃƒÂ¼rde) - das wÃƒÂ¼rde dazu fÃƒÂ¼hren, daÃƒÅ¸ ein B_Befehl, in einem Fall in dem
