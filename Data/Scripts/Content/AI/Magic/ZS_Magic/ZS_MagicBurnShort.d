...
{
	Npc_PercEnable(self, PERC_ASSESSMAGIC, B_AssessMagic); // falls in diesem Frame (wÃƒÂ¤hrend des AusfÃƒÂ¼hrens des B_) noch ein neuer Spruch wirkt
	...
	{
		B_AssessDamage();
		AI_ContinueRoutine(self); // Sicherheitshalber, falls B_AssessDamage NICHT in neuen ZS weiterleitet (kann z.ZT. aber nicht passieren) // - sonst wÃƒÂ¼rde NPC in Loop hÃƒÂ¤ngenbleiben
	}
	else
	{
...
func void B_RestartBurnShort()
{
	...
	|| (Npc_GetLastHitSpellID(self) == SPL_IceCube))
	{
		...
		Npc_ClearAIQueue(self);
		B_ClearPerceptions(self); // schaltet alle Wahrnehmungen ab - so kann keine spÃƒÂ¤ter priorisierte diesen Stateaufruf verhindern (s. z.B. AssessFightSound + AssessDamage)
...
func int ZS_MagicBurnShort()
{
	...
	// keine fire victim ani fÃƒÂ¼r nur kurz getroffen
...
func int ZS_MagicBurnShort_Loop()
{
	...
	// allerdings brennt ein hÃƒÂ¶her gecasteter Firespell auch lÃƒÂ¤nger und zieht somit sowieso mehr Schaden ab.
	...
	if (self.attribute[ATR_HITPOINTS] <= 0)
	{
		Npc_ClearAIQueue(self);
		AI_StandUp(self); // FIXME: wieso ? dann steht er doch noch mal auf bevor er abkratzt
		// (dann sollte er auch noch sowas sagen wie: "Macht das nicht zu Hause Kinder" bevor er endgÃƒÂ¼ltig stirbt :)
		return LOOP_END;
	};
