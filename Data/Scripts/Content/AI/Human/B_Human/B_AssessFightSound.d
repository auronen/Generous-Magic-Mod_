...
func void B_AssessFightSound()
{
	...
	// ------ im KLOSTER: wenn HÃƒÂ¶henunterschied zu groÃƒÅ¸ -------
	...
	// ------ FightSound wird NUR bei anderen NSCs (nicht self) berÃƒÂ¼cksichtigt ------
	...
	// ------ TÃƒÂ¤ter Monster (Opfer Human wegen oberer Abfrage) ------
	if ((other.guild > GIL_SEPERATOR_HUM)
	...
	// ------ Opfer Monster (TÃƒÂ¤ter Human wegen oberer Abfrage) ------
	...
	// ---------------------------------------------
	// IM FOLGENDEN sind TÃƒÂ¤ter und Opfer BEIDE Human
	// ---------------------------------------------

	// ---------------------------
	// WAS ist ein FakeBanditDuel?
	// ---------------------------
	// --> Player wird als Bandit ANGESEHEN und kÃƒÂ¤mpf gegen Bandit
	// ACHTUNG: Player muÃƒÅ¸ nicht nur BanditenrÃƒÂ¼stung anhaben,
	// sondern muÃƒÅ¸ auch vom NPC 'self' als Bandit ANGESEHEN werden
	...
	&& ((other.guild == GIL_BDT) || (victim.guild == GIL_BDT)))
	{
		...
		if (self.guild == GIL_BDT)
		{
			...
			// ------ FakeBandit-Player ist TÃƒÂ¤ter ------
			if ((self.aivar[AIV_STORYBANDIT_ESTEBAN] == TRUE)
	...
	// ----------------------------------------------------------------
	// ------ Wachen kommen zum TÃƒÂ¶ten (helfen other) ------------------
	// ----------------------------------------------------------------
	...
	&& (Npc_GetAttitude(self, other) == ATT_FRIENDLY))
	{
		// ------ Pack schlÃƒÂ¤gt sich, Pack vertrÃƒÂ¤gt sich -------
	...
	&& (self.guild == GIL_MIL)) // (!) NUR Miliz-Wache schlichtet diese Art KÃƒÂ¤mpfe
	{
		// ------ Freund helfen (other) ------
		...
		else
		{
			...
			else // beides keine Player -> Auf jeden Fall TÃƒÂ¤ter angreifen!
			{
				B_Attack(self, other, AR_GuardStopsFight, 0);
				return;
			};
	...
	// ------ NUR in Watchfight gehen, wenn nÃƒÂ¤her als 10m ------ (wichtig z.B. fÃƒÂ¼r GIL_OUT)
