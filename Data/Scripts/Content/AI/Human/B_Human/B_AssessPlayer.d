...
func void B_AssessPlayer()
{
	...
	// ------ Ignorieren, wenn SC tot, ohnmÃƒÂ¤chtig oder in magischem Schlaf ------
	...
	{
		if (C_NpcIsGateGuard(self)) // Wachen mÃƒÂ¼ssen auch freudliches/neutrales SC-Monster aufhalten
		{
	...
	// -------- Spieler hat Banditen RÃƒÂ¼stung an ------
	...
	// ------ SC ist in einem Portalraum, der mir oder meinen Freuden gehÃƒÂ¶rt ------
	...
	// ------------------------------------
	// Joly: ImportantInfos fÃƒÂ¼r AmbientDMT!
	B_AssignDementorTalk(self);
	// ------------------------------------

	// ------ Hat NSC Important Info fÃƒÂ¼r Spieler? Auch GUARDPASSAGE ------
	if ((Npc_GetDistToNpc(self, other) <= PERC_DIST_DIALOG)
	&& (Npc_CheckInfo(self, 1))) // REIHENFOLGE nicht vertauschen! (Check ist dahingehend buggy (darf nicht als erste Abfrage in &&-verknÃƒÂ¼pfter if-Abfrage stehen))
	{
		...
		{
			// ------- Spieler fÃƒÂ¤llt, schwimmt und taucht NICHT ------
	...
	// ------ sonst den Spieler einfach grÃƒÂ¼ÃƒÅ¸en (wenn ich ihm entgegenkomme) ------
	...
	&& (Npc_RefuseTalk(other) == FALSE) // Damit kein GruÃƒÅ¸-Maschinengewehr losgeht
	&& (!C_NpcIsGateGuard(self)) // GateGuards benutzen RefuseTalk-Counter, um zu verhindern, daÃƒÅ¸ der Spieler von hinten kommend auch angequatscht wird
	&& (C_PlayerHasFakeGuild(self, other) == FALSE))
	{
