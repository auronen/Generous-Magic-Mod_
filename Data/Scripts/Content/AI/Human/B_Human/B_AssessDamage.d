...
// B_AssessDamage
// --------------
// wird durchWahrnehmung PERC_AssessDamage ÃƒÅ“BERALL her aufgerufen
// (also auch aus ZS_Attack, AR kann sich allerdings NICHT ÃƒÂ¤ndern!)
// ****************************************************************
	...
	|| (Npc_IsInFightMode(other, FMODE_NONE))) // oder Waffe wieder weggesteckt (nicht ermittelbar)
	{
		...
		|| ((self.npctype == NpcType_Friend) && Npc_IsPlayer(other)))
		{
			...
			if (!Npc_IsInState(self, ZS_ReactToDamage))
			{
				...
				return; // im unterern B_Attack wird ein ClearQueue aufgerufen, der den AI_StartState direkt wieder aus der queue lÃƒÂ¶scht
				// - sicherheithalber bei allen AI_StartState-Aufrufen return anfÃƒÂ¼gen
			};
		};
	};
