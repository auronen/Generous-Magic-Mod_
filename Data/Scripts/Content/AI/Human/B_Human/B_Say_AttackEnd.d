...
func void B_Say_AttackEnd()
{
	...
	|| (self.aivar[AIV_ATTACKREASON] == AR_MonsterMurderedHuman))
	{
		if (other.guild < GIL_SEPERATOR_HUM)
		{
			...
			{
				B_Say(self, other, "$ENEMYKILLED"); // Das war's fÃƒÂ¼r dich, Penner.
			};
		}
		else
		{
	...
	if (self.aivar[AIV_ATTACKREASON] == AR_HumanMurderedHuman)
	{
		if (!Npc_IsDead(other))
		{
			if (self.aivar[AIV_LASTTARGET] == Hlp_GetInstanceID(other))
			{
				B_Say(self, other, "$KILLMURDERER"); // Stirb, MÃƒÂ¶rder!
			}
			else
			{
				B_Say(self, other, "$GOODKILL"); // (ruft) Ja, mach das Schwein fertig!
	...
	{
		B_Say(self, other, "$STUPIDBEASTKILLED"); // So ein saublÃƒÂ¶des Vieh!
		return;
	};
	...
	{
		B_Say(self, other, "$YOUBETTERSHOULDHAVELISTENED"); // Du hÃƒÂ¤ttest auf mich hÃƒÂ¶ren sollen!
		return;
	};
	...
	|| (self.aivar[AIV_ATTACKREASON] == AR_GuardCalledToRoom))
	{
		...
		{
			B_Say(self, other, "$NEVERENTERROOMAGAIN"); // Und daÃƒÅ¸ ich dich nie wieder da drin erwische!
		};
	...
	if (self.aivar[AIV_ATTACKREASON] == AR_GuardStopsFight)
	{
		...
		{
			B_Say(self, other, "$THEREISNOFIGHTINGHERE"); // Hier wird nicht gekÃƒÂ¤mpft, klar!? Lass dir das eine Lehre sein!
		};
