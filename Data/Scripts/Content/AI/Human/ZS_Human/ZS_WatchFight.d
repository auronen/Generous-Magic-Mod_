...
// (beide NICHT hostile zu self, sonst wÃƒÂ¼rden sie angegriffen)
// ***********************************************************
...
func void B_CheerFight() // wird nur hier (lokal) benutzt
{
	...
	if (C_NpcIsToughGuy(self))
	{
		...
		// ------ Opfer ist mein Freund, TÃƒÂ¤ter NICHT ------
		...
		// ------ TÃƒÂ¤ter ist mein Freund, Opfer NICHT ------
		// ----- TÃƒÂ¤ter und Opfer sind BEIDE Freunde -ODER- beide KEINE Freunde von mir ------
		else
		{
...
func void B_AssessDefeat()
{
	...
	else // NSC ist Weichei
	{
		...
		else // Bei AR_NONE gibt's kein Petzen und auch keine BestÃƒÂ¼rzung!
		{
			B_Say(self, other, "$NOTBAD");
		};
	};
...
func void ZS_WatchFight()
{
	...
	&& (!Npc_IsInState(victim, ZS_Unconscious)) // ZS_Watchfight kann aufgerufen werden, wenn der ERSTE Schag den Gegner niederstreckt ODER u.U. NOCHMAL aufgerufen werden, wenn eine Combo auf einen bewuÃƒÅ¸tlos werdenden Typen niederprasselt
...
func int ZS_WatchFight_Loop()
{
	...
	// wenn nicht vorher die Abfrage unten (Es wird nicht mehr gekÃƒÂ¤mpft) aus der Loop aussteigt
	...
	// ------ Es wird nicht mehr gekÃƒÂ¤mpft (alle FÃƒÂ¤lle ausser Unconscious und Murder) ------
	...
	// ------ Wenn ich zu nah dran bin, zurÃƒÂ¼ckweichen ------
	...
	|| (Npc_GetDistToNpc(self, victim) <= WATCHFIGHT_DIST_MIN))
	{
		...
		{
			B_TurnToNpc(self, victim); // zurÃƒÂ¼ckweichen vor dem ANDEREN, der weiter weg ist (verdreht)
		}
		else
		{
			B_TurnToNpc(self, other); // zurÃƒÂ¼ckweichen vor dem ANDEREN, der weiter weg ist (verdreht)
		};
